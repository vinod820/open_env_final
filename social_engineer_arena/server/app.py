...
<body>
  ...
  <div class="wrap">
    <div class="topbar">
      ...
      <div class="panel">
        ...
        <div class="form-grid">
          <div><input value="AI suggested mode only" aria-label="Action mode" readonly /></div>
    ...
    <div class="grid">
      ...
      <div class="right-col">
        <div class="panel">
          ...
          <div class="small">1) Start episode -> 2) Suggest action -> 3) Submit and inspect reward</div>
        </div>
        <div class="panel">
          <div class="label">Action Editor</div>
          <div class="form-grid">
            <div><div class="label">Verdict</div><select id="verdict" disabled><option value="unknown">unknown</option><option value="safe">safe</option><option value="phishing">phishing</option><option value="pretexting">pretexting</option></select><div id="verdictErr" class="inline-error" aria-live="polite"></div></div>
            <div><div class="label">Safety Boundary</div><input id="safetyBoundary" value="Defensive analysis only." readonly /><div id="boundaryErr" class="inline-error" aria-live="polite"></div></div>
          </div>
          <div class="label">Explanation</div><textarea id="explanation" rows="4" placeholder="Click Suggest Action to generate this field..." readonly></textarea><div id="explanationErr" class="inline-error" aria-live="polite"></div>
          <div class="label">Cues Found (comma-separated)</div><input id="cuesFound" placeholder="Click Suggest Action to generate cues..." readonly /><div id="cuesErr" class="inline-error" aria-live="polite"></div>
          <div class="label">Response</div><textarea id="response" rows="3" placeholder="Click Suggest Action to generate response..." readonly></textarea>
          <div id="errorSummary" class="error-summary" role="alert"></div>
          <div class="actions"><button id="resetBtn" class="secondary" title="Start a new scenario episode and load the first message context.">Start Episode</button><button id="suggestBtn" class="ghost" title="Generate a model-based suggested action (verdict, cues, explanation, response).">Suggest Action</button><button id="stepBtn" class="primary" title="Submit the suggested action to score this turn and move to the next state.">Submit Action</button><button id="exportBtn" class="ghost" title="Download the current session timeline as a JSONL trace file.">Export Trace JSONL</button></div>
        </div>
  ...
  <script>
    ...
    function setBusy(v) { for (const id of ["resetBtn","stepBtn","suggestBtn"]) $(id).disabled = v; }
